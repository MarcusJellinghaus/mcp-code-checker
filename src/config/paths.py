...
def validate_windows_path(path: Path) -> tuple[bool, Optional[str]]:
    ...
    # Check path length
    if not path_str.startswith('\\\\?\\') and len(path_str) > PathConstants.WIN_MAX_PATH_CLASSIC:
        return False, f"Path exceeds Windows MAX_PATH limit ({PathConstants.WIN_MAX_PATH_CLASSIC} chars)"
...
def get_safe_path_for_platform(
    ...
    if sys.platform == 'win32' and len(str(full_path)) > PathConstants.WIN_MAX_PATH_CLASSIC:
        # Truncate base name to fit
        max_base_len = PathConstants.WIN_MAX_PATH_CLASSIC - len(str(directory)) - len(extension) - 2
...
def ensure_parent_directory(path: Path, mode: int = 0o755) -> None:
    ...
    if not parent.exists():
        try:
            ...
            if sys.platform == 'win32' and len(str(parent)) > PathConstants.WIN_MAX_PATH_CLASSIC:
                raise OSError(
