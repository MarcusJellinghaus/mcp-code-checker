...
class OutputFormatter:
    ...
    def print_auto_detected_params(params: dict[str, Any]) -> None:
        ...
        for key, value in params.items():
            if value is not None:
                ...
                if key == "python_executable" and (
                    ".venv" in value_str or "venv" in value_str
                ):
                    value_str += " (from venv)"
                elif key == "venv_path":
                ...
                prefix = (
                    OutputFormatter.TREE_BRANCH
                    if key != list(params.keys())[-1]
                    else OutputFormatter.TREE_LAST
                )
    ...
    def print_validation_results(validation_result: dict[str, Any]) -> None:
        ...
        if validation_result.get("success"):
            print(
                f"{OutputFormatter.CHECK_PASS} Configuration is valid and ready to use"
            )
        else:
    ...
    def print_configuration_details(
        ...
        if tree_format:
            ...
            for i, (key, value) in enumerate(param_items):
                ...
                prefix = (
                    OutputFormatter.TREE_LAST
                    if is_last
                    else OutputFormatter.TREE_BRANCH
                )
    ...
    def print_dry_run_config_preview(
        ...
        print()
        print(
            f"{OutputFormatter.CHECK_PASS} Configuration is valid. Run without --dry-run to apply changes."
        )
    ...
    def print_dry_run_remove_preview(
        ...
        print(
            f"{OutputFormatter.TREE_BRANCH} Server Type: {server_info.get('type', 'unknown')}"
        )
        print(
            f"{OutputFormatter.TREE_BRANCH} Command: {server_info.get('command', 'N/A')}"
        )
        ...
        print()
        print(
            f"{OutputFormatter.CHECK_PASS} Removal is safe. Run without --dry-run to apply changes."
        )
    ...
    def print_enhanced_server_list(
        ...
        if managed_servers:
            ...
            for i, server in enumerate(managed_servers):
                ...
                print(
                    f"{prefix} {server['name']} ({server.get('type', 'unknown')}) {status_icon}"
                )

                if detailed:
                    indent = (
                        OutputFormatter.TREE_SPACE
                        if is_last
                        else OutputFormatter.TREE_PIPE
                    )
                    if "command" in server:
                        print(
                            f"{indent} {OutputFormatter.TREE_BRANCH} Command: {server['command']}"
                        )
                    if server.get("args"):
                        args_str = " ".join(server["args"])
                        if len(args_str) > 60:
                        ...
                        print(
                            f"{indent} {OutputFormatter.TREE_BRANCH} Args: {args_str}"
                        )
        ...
        if external_servers:
            ...
            for i, server in enumerate(external_servers):
                ...
                print(
                    f"{prefix} {server['name']} (external) - not managed by mcp-config"
                )
        else:
