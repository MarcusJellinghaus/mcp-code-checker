...
class TestGetDirectInstructionForPylintCode:
    ...
    def test_known_codes(self) -> None:
        ...
        assert instruction_r0902 is not None and "Refactor the class" in instruction_r0902
        
        instruction_c0411 = get_direct_instruction_for_pylint_code("C0411")
        assert instruction_c0411 is not None and "Organize your imports" in instruction_c0411
        
        instruction_w0612 = get_direct_instruction_for_pylint_code("W0612")
        assert instruction_w0612 is not None and "use the variable or remove" in instruction_w0612
        
        instruction_w0621 = get_direct_instruction_for_pylint_code("W0621")
        assert instruction_w0621 is not None and "Avoid shadowing variables" in instruction_w0621
...
class TestGetPromptForKnownPylintCode:
    ...
    def test_known_code_with_messages(self) -> None:
        ...
        assert any(path in prompt for path in ["src/test.py", "src\\\\test.py", "src\\test.py"])
...
class TestGetPromptForUnknownPylintCode:
    ...
    def test_unknown_code_prompt(self) -> None:
        ...
        result = PylintResult(return_code=0, messages=messages)
        prompt = get_prompt_for_unknown_pylint_code("X9999", "/home/user/project", result)
    ...
    def test_multiple_messages_same_code(self) -> None:
        ...
        assert any(path in prompt for path in ["src/file1.py", "src\\\\file1.py", "src\\file1.py"])
        assert any(path in prompt for path in ["src/file2.py", "src\\\\file2.py", "src\\file2.py"])
