...
class TestVSCodeHandler:
    ...
    @pytest.mark.skipif(os.name == 'nt', reason="Linux path test doesn't work on Windows")
    ...
    @pytest.mark.skipif(os.name == 'nt', reason="macOS path test doesn't work on Windows")
    def test_user_config_path_macos(self):
        ...
        with patch('platform.system', return_value='Darwin'):
            with patch.object(os, 'name', 'posix'):
                ...
                assert 'Library' in path_str or 'Application Support' in path_str.replace('\\', '/')
                assert 'Code' in path_str
                assert 'mcp.json' in path_str
    ...
    def test_validate_config(self, tmp_path):
        ...
        with patch('src.config.clients.Path.cwd') as mock_cwd:
            ...
            assert len(errors) > 0
            assert any("missing required 'command' field" in e.lower() or "command" in e.lower() 
                      for e in errors)
