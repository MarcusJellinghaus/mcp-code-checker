...
def test_model_instantiation() -> None:
    ...
    traceback_entry = TracebackEntry(path="/path/to/file.py", lineno=10, message="Error message")
    assert traceback_entry.path == "/path/to/file.py"
    assert traceback_entry.lineno == 10
    assert traceback_entry.message == "Error message"

    test_stage = TestStage(duration=0.1, outcome="failed", crash=crash, traceback=[traceback_entry])
...
def test_report_model_structure() -> None:
    ...
    assert test.call.crash.message == "TypeError: unsupported operand type(s) for -: 'int' and 'NoneType'"
    ...
    assert report.warnings[0].message == "cannot collect test class 'TestFoo' because it has a __init__ constructor"
